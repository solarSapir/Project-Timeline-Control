...
const MERGE_MAP: Record<string, (p: Project) => string> = {
  ...
  "{{subtotal}}": (p) => (p as any).contractSubtotal || "",
  "{{hst_rate}}": (p) => {
  ...
  "{{hst_amount}}": (p) => (p as any).contractHstAmount || "",
  "{{total_price}}": (p) => (p as any).contractTotal || "",
  "{{helcim_link}}": (p) => (p as any).contractHelcimLink || "",
  "{{date}}": () => new Date().toLocaleDateString("en-CA"),
  "{{rep_name}}": (p) => (p as any).contractRepName || "",
  "{{signer_name}}": () => "",
