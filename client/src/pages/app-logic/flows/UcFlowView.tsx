...
const initialNodes: Node[] = [
  ...
  { id: "filter", type: "flowNode", position: { x: 300, y: 180 }, data: { label: "Filter Projects", type: "filter", description: "Only show relevant UC projects", items: ["installType = 'install'", "propertySector = 'residential' (or null)", "Exclude: complete, paused, lost", "Hide: escalation-hidden + workflow-hidden"] } },
  ...
  { id: "card", type: "flowNode", position: { x: 300, y: 360 }, data: { label: "UCProjectCard", type: "component", description: "Renders each project as a card", items: ["Status badge + due date indicator", "Hydro bill info section", "UC document checklist", "Subtask expand panel", "Escalation badge", "Status change dropdown"] } },

  { id: "status-change", type: "flowNode", position: { x: 0, y: 540 }, data: { label: "handleStatusChange()", type: "action", description: "Intercepts status dropdown changes", items: ["If 'Approved' → open ApprovalDialog", "If 'Rejected' → open RejectionDialog", "If 'Submitted' → PATCH + set ucSubmittedDate + log completion (7d hide)", "Otherwise → PATCH /api/projects/:id"] } },

  { id: "approval-dialog", type: "flowNode", position: { x: 0, y: 760 }, data: { label: "UcApprovalDialog", type: "dialog", description: "Modal when status → Approved", items: ["Fields: staff name, connection fee, files", "PATCH project (ucStatus=Approved)", "POST files to /api/projects/:id/files (category=UC)", "POST /api/uc/complete-action (hideDays=14)", "Invalidates projects + completions cache"] } },

  { id: "rejection-dialog", type: "flowNode", position: { x: 350, y: 760 }, data: { label: "UcRejectionDialog", type: "dialog", description: "Modal when status → Rejected", items: ["Fields: staff name, files, notes", "PATCH project status", "POST files (category=UC)", "POST /api/uc/complete-action", "Auto-creates escalation ticket", "Invalidates projects + completions + tickets"] } },

  { id: "followup-dialog", type: "flowNode", position: { x: 700, y: 540 }, data: { label: "FollowUpDialog", type: "dialog", description: "Follow-up for submitted projects (7+ days)", items: ["Fields: 'What has been done' + 'Next Steps'", "Posts comment to UC subtask via Asana API", "POST /api/uc/complete-action (follow_up, 7d hide)", "Invalidates completions cache"] } },

  { id: "hide-logic", type: "flowNode", position: { x: 700, y: 360 }, data: { label: "Hide/Reappear Logic", type: "logic", description: "Projects hidden after workflow actions", items: ["Latest completion.completedAt + hideDays > now → hidden", "7 days for submitted, 14 days for approved", "'Hidden' filter shows hidden projects", "Escalation hide: 48h from ticket creation"] } },

  { id: "hydro-section", type: "flowNode", position: { x: 700, y: 760 }, data: { label: "HydroInfoSection", type: "component", description: "Hydro bill upload + AI extraction", items: ["Upload image → POST /api/projects/:id/hydro-bill", "OpenAI Vision extracts: company, account#, customer", "Manual edit fields also available", "Stored on project record"] } },

  { id: "doc-checklist", type: "flowNode", position: { x: 1000, y: 540 }, data: { label: "UcDocChecklist", type: "component", description: "Document requirements on each card", items: ["(1) Hydro Bill / Account# — links to hydro upload", "(2) Meterbase Photo — POST /api/projects/:id/meterbase", "Meterbase required for Alectra, optional otherwise", "Legacy statuses → grayed out (assumed collected)"] } },
];
