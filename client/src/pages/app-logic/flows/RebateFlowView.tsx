...
const initialNodes: Node[] = [
  ...
  { id: "status-change", type: "flowNode", position: { x: 0, y: 520 }, data: { label: "handleRebateStatus()", type: "action", description: "Status dropdown change handler", items: ["PATCH /api/projects/:id with rebateStatus", "If 'Close-off': also sets rebateCloseOffDate", "Invalidates projects cache", "Pushes to Asana via backend"] } },
  ...
  { id: "hrsp-checklist", type: "flowNode", position: { x: 0, y: 700 }, data: { label: "HrspChecklist", type: "component", description: "Two-phase document tracking", items: ["Pre-Approval: Invoice, Participation Doc, Hydro Bill, SLD", "Close-Off: Roof Photos, Panel/Inverter/Battery Nameplates, ESA Cert, Paid Invoice", "Pre-approval grayed when post-approval", "Each doc: upload button → POST /api/projects/:id/hrsp-*", "Invoice/Paid Invoice: generate PDF dialogs"] } },

  { id: "followup", type: "flowNode", position: { x: 300, y: 700 }, data: { label: "Follow-Up System", type: "logic", description: "5-day follow-up cycle", items: ["rebateSubmittedDate tracks status change", "In-progress or Submitted → 5d cycle", "Cards show amber border when due", "RebateFollowUpDialog: action taken + next steps", "Posts to HRSP subtask (not main task)", "POST /api/rebate/push-followup"] } },
