...
import { Search, AlertTriangle, CheckCircle2, Clock, CalendarIcon, Camera, Lock } from "lucide-react";
...
export default function AHJView() {
  ...
  const depsMetInstallProjects = installProjects.filter((p: any) =>
    areDependenciesMet(p, "ahj_permitting", workflowConfigs)
  );
  const waitingDepsProjects = installProjects.filter((p: any) =>
    !areDependenciesMet(p, "ahj_permitting", workflowConfigs) && !isAhjComplete(p.ahjStatus)
  );

  const ahjProjects = depsMetInstallProjects.map((p: any) => {
