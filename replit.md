...
- **Document Template System**: Upload PDF/image files as templates, visually place fillable fields on them in Settings, and auto-generate completed documents for any project. Templates are organized by view type (UC, Contracts, Rebates, etc.). Fields support text/date/number/select types with font customization, auto-fill from project data, and required validation. Generated documents are saved as project files. Template files stored in `data/templates/` with DB backup in `document_templates.fileData`. PDF generation uses `pdf-lib` for PDF templates and `pdfkit` for image templates.
