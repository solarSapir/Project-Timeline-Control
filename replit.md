...
## Recent Changes
- 2026-02-24: Dependency Gating — workflow config dependencies now enforce which projects appear in each view. If Contract depends on UC+Rebates, projects won't show in Contracts tab until both are met. Each view has "Waiting on Dependencies" filter showing blocked projects with unmet criteria. Settings page shows "Required to unlock" labels with completion criteria per dependency. Shared utility: `client/src/lib/stage-dependencies.ts` with `isStageComplete()`, `areDependenciesMet()`, `getUnmetDependencies()`, and `STAGE_COMPLETION_CRITERIA` map.
- 2026-02-24: Gantt Chart improved — starts from earliest expected date (not project creation), "Zoom to today" toggle, colors match Settings page (blue/purple/amber/emerald/orange/cyan/indigo/rose), date labels instead of day numbers.
...
- 2026-02-24: Workflow Configuration — Settings page has visual workflow editor showing stage dependency chain (which stages depend on others, which run in parallel). Editable gap days per stage, editable "gap relative to" dropdown (choose which dependency the gap is measured from), editable dependency checkboxes (toggle which stages each stage depends on). Cumulative timeline overview bar chart. `gapRelativeTo` column in `workflow_config` DB table. API: GET/PUT /api/workflow-config.
