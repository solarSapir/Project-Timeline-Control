...
- **Pause Reason Tracking**: Paused project cards have expandable "Reason for Pause" section with dropdown of common reasons (stored in `pause_reasons` table, grows over time), custom reason entry, and free-text note. AI-powered insights in Settings analyze pause patterns and generate recommendations.
- **Project Planner View**: New `/planner` page between AHJ and Install Coordination in sidebar. Full pre-install preparation checklist: Confirm Scope of Work, Upload Original Proposal, Total Project Cost (customer billing amount), Contractor Payout Amount, Contractor Assignment (syncs to Asana), Contractor Contract Sent, Contractor Contract Signed, and Electrical Permit (NS-only). Install projects are hidden from all UC view filters until the full planner checklist is complete; they appear under "Awaiting Planner" filter in UC view. Schema fields: `plannerScopeConfirmed`, `plannerProposalUrl`, `plannerTotalCost`, `plannerContractorPayout`, `plannerContractSent`, `plannerContractSigned`, `electricalPermitUrl`.

### Database Tables
- `users`, `projects`, `project_deadlines`, `task_actions`, `install_schedule`, `workflow_config`, `error_logs`, `hrsp_config`, `project_files`, `escalation_tickets`, `uc_completions`, `uc_workflow_rules`, `rebate_completions`, `rebate_workflow_rules`, `staff_members`, `pause_reasons`.
