...
- **KPI Consistency Rule**: All KPI sections (UC, Rebate, Contract, Escalation) must include FormulaTooltips on every metric card. When adding tooltips/drilldowns/improvements to one KPI section, apply the same to all KPI sections. All KPI sections use `CollapsibleKpiSection` wrapper with localStorage-persisted collapse state and summary preview when collapsed.
...
- **Document Template System**: Two template types: (1) **Overlay templates** — upload PDF/image files, visually place fillable fields on them in Settings, and auto-generate completed documents by overlaying text at field positions. (2) **Editable contract templates** — rich text editor (TipTap) for creating/editing contracts like a word processor, with import from .docx files (mammoth.js), full formatting (bold/italic/underline/headings/lists/tables/alignment/color), merge field placeholders (e.g. `{{customer_name}}`), and electronic signature support (drawn or typed, legally compliant with Canada's PIPEDA). Templates organized by view type (UC, Contracts, Rebates, etc.). `document_templates.templateType` column distinguishes "overlay" vs "editable"; `htmlContent` column stores editable template HTML. PDF generation: overlay templates use `pdf-lib`/`pdfkit`; editable contracts use client-side `html2pdf.js` for HTML-to-PDF conversion with signatures embedded. Signature audit data (signer name, method, timestamp) recorded in project file notes. Template files stored in `data/templates/` with DB backup in `document_templates.fileData`. **Contract Details fields** stored on projects table: `clientPhone`, `clientEmail`, `projectAddress`, `projectDescription`, `contractSubtotal`, `contractHstAmount`, `contractTotal`, `contractHelcimLink`, `contractRepName` — editable inline from ContractExpandedView, auto-populate MERGE_MAP during contract generation. Designed for future CRM integration to pull these values automatically.
